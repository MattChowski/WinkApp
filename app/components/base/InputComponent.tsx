  ...
  ({ label, description, icon: Icon, error = false, errorMessage, intent, className, ...props }, inputRef) => {
    return (
      <Field className="w-full">
        {label && <Label className="mb-2 font-semibold text-sm text-tertiary">{label}</Label>}
        {description && <Description className="font-light text-sm text-tertiary/75">{description}</Description>}
        ...
        {errorMessage && <p className="mt-1 text-red-400 text-sm">{errorMessage}</p>}
      </Field>
    )
  },
)
